...
export function probeVideo(filePath: string, ffprobePath: string): Promise<ProbeResult> {
  return new Promise((resolve, reject) => {
    ...
    p.on("close", code => {
      ...
      } catch (e) { reject(new Error(`Không parse được video metadata: ${(e as Error).message}`)); }
    });
  });
}